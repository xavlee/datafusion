...
### `FFI_PlanProperties` includes group-contiguous expressions

`FFI_PlanProperties` now appends a `group_contiguous_exprs` callback that
returns the components of an execution plan's group-contiguous composite key.
This changes the `#[repr(C)]` struct layout. FFI producers and consumers must
therefore both use the DataFusion 55 layout and be rebuilt and deployed
together; mixing a pre-55 layout with the DataFusion 55 layout is not ABI
compatible.

Code that constructs `FFI_PlanProperties` directly must initialize the new
callback and return an empty vector when no such guarantee is available. Rust
execution plans publish the property through
`PlanProperties::with_group_contiguous_exprs`.
